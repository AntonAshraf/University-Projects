...
{
  if (r1 != r2 || c1 != c2)
  { // check if they are equals in rows and coulumns
    Red();
...
void printBoard()
{
  ...
  for (i = 0; i < R; i++)
  {
    ...
    for (j = 0; j <= C; j++)
    {
      ...
      else if (j == C / 2 - 1 && i > 3 && i < 22 && choice == '3')
      { // middle coulmn
        printf("%c", 186);
      }
...
void movement()
{
  ...
  if ((userInput == 'w' || userInput == 'a' || userInput == 's' || userInput == 'd') && abs(direction - userInput) > 5)
  {
    direction = userInput;
  }
  if (direction == 'w')
